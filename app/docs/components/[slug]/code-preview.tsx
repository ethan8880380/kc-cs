...
function highlightCode(code: string) {
  ...
  return lines.map((line, lineIndex) => {
    ...
    while (i < line.length) {
      ...
      const keywordMatch = line.slice(i).match(/^(import|export|from|function|return|const|let|var|interface|type|extends|typeof|as|default|npx)(?=\s|$|{|\(|<|,)/)
      if (keywordMatch) {
        tokens.push(
      ...
      let plainText = ""
      while (i < line.length && 
             !line.slice(i).match(/^(import|export|from|function|return|const|let|var|interface|type|extends|typeof|as|default|npx)(?=\s|$|{|\(|<|,)/) &&
